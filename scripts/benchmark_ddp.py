...
def run_ddp(world_size: int=2, vocab_size = 10000, batch_size = 4, context_length = 512):
    ...
    mp.spawn(fn=benchmark_ddp, args=(world_size, device, x, y, vocab_size, context_length, batch_size), nprocs=world_size, join=True)
...
def run_ddp_nsys():
    ...
    cmd = [
        "nsys", "profile",
        "-o", "/root/data/ddp_profile_overlap",
...
def main(profile: bool = False):
    ...
    print(f"Benchmarking Naive DDP....")
    if profile:
        run_ddp_nsys.remote()
    else:
...
if __name__ == "__main__":
    ...
    mp.spawn(fn=benchmark_ddp, args=(world_size, "cuda", x, y, vocab_size, context_length, batch_size), nprocs=world_size, join=True)